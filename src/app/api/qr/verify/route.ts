...
export async function POST(request: Request) {
  try {
    ...
    // auth nesnesinin null olmadığını kontrol ettikten sonra getUser metodunu çağır
    let generatorUsername = 'unknown';
    if (auth) {
      try {
        const generatorUserRecord = await auth.getUser(tokenData.generatorUid);
        generatorUsername = generatorUserRecord.email?.split('@')[0] || 'unknown'; // veya profil bilgisinden al
      } catch (error) {
        console.error('Error fetching generator user:', error);
      }
    }
